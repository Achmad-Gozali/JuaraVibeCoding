...
app.use('*', cors({
  origin: (origin, c) => {
    const allowed = ['http://localhost:3000', 'http://localhost:3001', 'https://kawaltransaksi.com', 'https://kawaltransaksi-605520424162.asia-southeast1.run.app', c.env.FRONTEND_URL].filter(Boolean);
    if (!origin) return '*';
