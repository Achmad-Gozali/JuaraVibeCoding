...
search.use("*", async (c, next) => {
  if (!c.env.LIMITER) {
    console.warn(
      "[SEARCH RATE LIMIT] KV LIMITER tidak tersedia. Rate limiting dinonaktifkan.",
    );
    return next();
  }
  ...
  try {
    if (isLoggedIn) {
        ...
        return c.json(
          {
            success: false,
            message:
              "Batas pencarian tercapai (15/menit). Tunggu sebentar lalu coba lagi.",
          },
          429,
        );
      }
      ...
      await Promise.all([
        c.env.LIMITER.put(userKey, (userHits + 1).toString(), {
          expirationTtl: 60,
        }),
        c.env.LIMITER.put(ipKey, (ipHits + 1).toString(), {
          expirationTtl: 60,
        }),
      ]);
    } else {
        ...
        return c.json(
          {
            success: false,
            message:
              "Batas pencarian tercapai (5/menit). Login untuk mendapatkan kuota lebih banyak, atau tunggu sebentar.",
          },
          429,
        );
      }

      await c.env.LIMITER.put(ipKey, (ipHits + 1).toString(), {
        expirationTtl: 60,
      });
    }
  } catch (err) {
...
search.post("/verify-turnstile", async (c) => {
  try {
    ...
    if (!isValid) {
      return c.json(
        { success: false, message: "Verifikasi keamanan gagal." },
        400,
      );
    }
