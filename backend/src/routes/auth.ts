...
async function checkRateLimit(
  ...
  try {
    ...
    await limiter.put(key, (count + 1).toString(), {
...
auth.post("/register", async (c) => {
  try {
    ...
    if (
      ...
      return c.json(
        { success: false, message: "Verifikasi keamanan wajib diselesaikan." },
        400,
      );
    }
    ...
    if (password.length < 8) {
      return c.json(
        {
          success: false,
          message: "Kata sandi tidak memenuhi persyaratan keamanan.",
        },
        400,
      );
    }
    ...
    if (c.env.LIMITER) {
      ...
      const registerCheck = await checkRateLimit(
        c.env.LIMITER,
        registerKey,
        3,
        600,
      ...
      if (registerCheck.blocked) {
        ...
        // Auto blacklist kalau abuse register
        await autoBlacklistIfAbuse(
          c.env.LIMITER,
          ip,
          "Abuse pada endpoint register",
        );
        ...
        return c.json(
          {
            success: false,
            message:
              "Terlalu banyak percobaan pendaftaran. Tunggu beberapa menit.",
            retry_after: 600,
          },
          429,
        );
    ...
    const blacklistCheck = await checkAndBlacklistTurnstile(
      c.env.LIMITER,
      turnstileToken,
    );
    ...
    if (!isValidTurnstile) {
      return c.json(
        {
          success: false,
          message: "Verifikasi keamanan gagal. Muat ulang halaman.",
        },
        400,
      );
    }
      ...
      return c.json(
        {
          success: false,
          message:
            "Terjadi kesalahan saat mendaftar. Coba gunakan email lain atau masuk jika sudah punya akun.",
        },
        409,
      );
    }
    ...
    if (linkError || !linkData) {
      ...
      return c.json(
        {
          success: false,
          message: "Gagal mengirim email verifikasi. Coba lagi.",
        },
        500,
      );
    }
...
auth.post("/login", async (c) => {
  try {
    ...
    if (c.env.LIMITER) {
      const emailSlug = btoa(normalizedEmail)
        .replace(/[^a-zA-Z0-9]/g, "")
        .slice(0, 20);
      ...
      if (comboCheck.blocked) {
        ...
        // Auto blacklist kalau abuse login
        await autoBlacklistIfAbuse(
          c.env.LIMITER,
          ip,
          "Abuse pada endpoint login",
        );
      ...
      if (emailCheck.blocked) {
        console.warn(
          `[LOGIN RL] Email diblokir dari semua IP — email: ${normalizedEmail}`,
        );
        return c.json(
          {
            success: false,
    ...
    if (signInError) {
      ...
      if (currentAttempts >= MAX_ATTEMPTS) {
        ...
        // Auto blacklist kalau sudah lockout berkali-kali
        if (c.env.LIMITER) {
          await autoBlacklistIfAbuse(
            c.env.LIMITER,
            ip,
            "Berulang kali gagal login hingga lockout",
          );
        }
