...
// .wrangler/tmp/bundle-CZaT3c/checked-fetch.js
...
auth.post("/login", async (c) => {
  try {
    ...
    const { data: profile } = await supabaseAdmin.from("profiles").select("failed_attempts, locked_until, is_banned").eq("id", authUser.id).single();
    ...
    if (signInError) {
      ...
      if (currentAttempts >= MAX_ATTEMPTS) {
        const lockedUntil = new Date(Date.now() + LOCK_DURATION_MINUTES * 60 * 1e3);
        ...
        return c.json({
          ...
          locked: true,
          locked_until: lockedUntil.toISOString(),
          remaining_ms: LOCK_DURATION_MINUTES * 60 * 1e3
        }, 429);
      } else {
        await supabaseAdmin.from("profiles").update({ failed_attempts: currentAttempts }).eq("id", authUser.id);
        return c.json({
          success: false,
          message: remaining === 1 ? "Kata sandi salah. Tersisa 1 percobaan sebelum akun dikunci." : `Kata sandi salah. Tersisa ${remaining} percobaan lagi.`,
          locked: false,
...
__name(parseGroqJson, "parseGroqJson");
async function analyzeEvidenceImage(base64Image, mimeType, apiKey) {
...
__name(analyzeEvidenceImage, "analyzeEvidenceImage");
async function analyzeChronologyText(chronology, apiKey) {
  const response = await fetch(GROQ_API_URL, {
    method: "POST",
    headers: {
      Authorization: `Bearer ${apiKey}`,
...
__name(analyzeChronologyText, "analyzeChronologyText");
...
__name(determineAutoStatus, "determineAutoStatus");
reports.post("/", authMiddleware, async (c) => {
  try {
    ...
    const { count: todayCount } = await supabase.from("reports").select("*", { count: "exact", head: true }).eq("reporter_id", userId).gte("created_at", oneDayAgo);
    if ((todayCount ?? 0) >= 10) {
    ...
    const sanitizedSocialAccounts = body.social_media_accounts ? sanitizeArray(body.social_media_accounts) : [];
    const evidenceUrls = body.evidence_urls?.length > 0 ? body.evidence_urls : body.evidence_url ? [body.evidence_url] : [];
    ...
    try {
      ...
      const photoResult = body.ai_photo_result ? { ...body.ai_photo_result, summary: "", red_flags: [], scam_category_suggestion: null } : null;
...
reports.post("/withdraw", authMiddleware, async (c) => {
  try {
    ...
    const supabase = getSupabaseAdmin(c.env);
    const { data: report, error: fetchError } = await supabase.from("reports").select("id, status, reporter_id").eq("id", reportId).eq("reporter_id", userId).single();
    ...
    const { error } = await supabase.from("reports").update({ status: "withdrawn" }).eq("id", reportId).eq("reporter_id", userId);
...
reports.put("/:reportId", authMiddleware, async (c) => {
  try {
    ...
    const { error } = await supabase.from("reports").update(sanitizedData).eq("id", reportId).eq("reporter_id", userId);
    if (error) throw error;
...
admin.get("/users", authMiddleware, requireAdmin, async (c) => {
  try {
    const supabase = getSupabaseAdmin(c.env);
    const { data: profiles } = await supabase.from("profiles").select("id, full_name, role, is_banned, created_at, updated_at").order("created_at", { ascending: false });
...
// .wrangler/tmp/bundle-CZaT3c/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-CZaT3c/middleware-loader.entry.ts
