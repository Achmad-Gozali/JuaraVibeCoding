...
function determineAutoStatus(params: {
  ...
  if (hasPhoto && photoResult) {
    ...
    if (!photoAuthOk) {
      return { status: 'pending', reason: `foto mencurigakan (keaslian ${photoResult.authenticity_score}/100)` };
    }
    if (!photoRelOk) {
      return { status: 'pending', reason: `foto tidak relevan (relevansi ${photoResult.relevance_score}/100)` };
    }
  ...
  if (!hasPhoto && riskLevel === 'high' && chronologyScore >= 85) {
    return { status: 'verified', reason: `kronologi high-risk sangat detail (skor ${chronologyScore}) tanpa foto` };
  }
  return { status: 'pending', reason: 'tidak memenuhi semua syarat auto-verifikasi' };
}
...
export async function submitReport(formData: {
  ...
  if (formData.link_url) {
    try {
      ...
      return { success: false, error: 'format link URL tidak valid.' };
    }
  }
  ...
  try {
    ...
    // Fallback ke pending kalau AI error
  }
