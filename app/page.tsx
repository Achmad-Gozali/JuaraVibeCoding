...
export default async function HomePage() {
  ...
  const [
    ...
    supabase.from('reports').select('id, target_number, target_name, category, created_at, status').order('created_at', { ascending: false }).limit(6),
    supabase.rpc('get_category_counts'),
  ]);

  const recentReports = (rawRecentReports as any[] | null) ?? [];
  const sortedCategories = ((rawCategoryData as any[] | null) ?? []).slice(0, 6).map((r: any) => [r.category, Number(r.count)] as [string, number]);
    ...
    <div>
        ...
        <div className="relative max-w-4xl mx-auto px-4 pt-14 pb-16 sm:pt-20 sm:pb-24 text-center">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, ease: [0.16, 1, 0.3, 1] }}>
            ...
            <p className="text-base sm:text-lg text-zinc-500 mb-8 sm:mb-10 max-w-xl mx-auto leading-relaxed px-2">
              Verifikasi nomor HP atau rekening bank sebelum bertransaksi. Gratis, cepat, dan didukung oleh komunitas.
            </p>
          </motion.div>
          ...
          >
            <Link href="/cek-nomor" className="inline-flex items-center justify-center gap-3 px-6 sm:px-8 py-4 rounded-2xl bg-zinc-900 text-white text-sm sm:text-base font-bold hover:bg-black transition-all active:scale-95 shadow-lg shadow-zinc-900/10">
              <Phone className="w-5 h-5" />Cek Nomor HP
            </Link>
            <Link href="/cek-rekening" className="inline-flex items-center justify-center gap-3 px-6 sm:px-8 py-4 rounded-2xl bg-white text-zinc-900 text-sm sm:text-base font-bold border border-zinc-200 hover:border-zinc-400 hover:bg-zinc-50 transition-all active:scale-95 shadow-sm">
              <Landmark className="w-5 h-5" />Cek Rekening Bank
            </Link>
          </motion.div>
          ...
          >
            <div className="text-center">
              <p className="text-2xl sm:text-3xl font-extrabold text-zinc-900">{totalReports || 0}</p>
              <p className="text-[10px] sm:text-xs text-zinc-400 font-medium mt-1 uppercase tracking-wider">Total Laporan</p>
            </div>
            <div className="w-px bg-zinc-200" />
            <div className="text-center">
              <p className="text-2xl sm:text-3xl font-extrabold text-zinc-900">{verifiedCount || 0}</p>
              <p className="text-[10px] sm:text-xs text-zinc-400 font-medium mt-1 uppercase tracking-wider">Terverifikasi</p>
            </div>
            <div className="w-px bg-zinc-200" />
            <div className="text-center">
              <p className="text-2xl sm:text-3xl font-extrabold text-zinc-900">24/7</p>
              <p className="text-[10px] sm:text-xs text-zinc-400 font-medium mt-1 uppercase tracking-wider">Monitoring</p>
            </div>
          </motion.div>
        </div>
      </section>
      ...
      <section className="py-14 sm:py-20 bg-white border-b border-zinc-100">
        <div className="max-w-5xl mx-auto px-4">
          <div className="text-center mb-10 sm:mb-14">
            ...
            <h2 className="text-2xl sm:text-3xl md:text-4xl font-extrabold text-zinc-900 tracking-tight">Mudah, Cepat, 3 Langkah.</h2>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 sm:gap-8">
            {[
              ...
              >
                <div className={`w-12 h-12 sm:w-16 sm:h-16 rounded-2xl border-2 ${item.color} flex items-center justify-center shrink-0 sm:mb-5 bg-white`}>
                  <item.icon className="w-5 h-5 sm:w-7 sm:h-7" />
                </div>
                <div>
                  <span className="text-[10px] font-bold text-zinc-300 uppercase tracking-widest block mb-1">{item.step}</span>
                  <h3 className="text-sm sm:text-base font-bold text-zinc-900 mb-1 sm:mb-2">{item.title}</h3>
      ...
      <section className="py-14 sm:py-20 bg-zinc-50">
        <div className="max-w-6xl mx-auto px-4">
          <div className="text-center mb-10 sm:mb-14">
            <p className="text-[11px] font-bold uppercase tracking-[0.2em] text-zinc-400 mb-3">Mengapa KawalTransaksi?</p>
            <h2 className="text-2xl sm:text-3xl md:text-4xl font-extrabold text-zinc-900 tracking-tight">
              Perlindungan dari komunitas,<br className="hidden sm:block" />untuk komunitas.
            </h2>
          </div>
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-5">
            {[
              ...
              >
                <div className={`w-9 h-9 sm:w-10 sm:h-10 ${item.color} rounded-xl flex items-center justify-center mb-3 sm:mb-4`}>
                  <item.icon className="w-4 h-4 sm:w-5 sm:h-5" />
                </div>
                <h3 className="text-sm font-bold text-zinc-900 mb-1 sm:mb-1.5">{item.title}</h3>
                <p className="text-xs sm:text-sm text-zinc-500 leading-relaxed">{item.desc}</p>
              </motion.div>
            ))}
          </div>
          ...
          <div className="max-w-4xl mx-auto px-4">
            <div className="flex flex-col sm:flex-row sm:items-end justify-between mb-8 sm:mb-10 gap-3">
              <div>
                ...
                <h2 className="text-2xl sm:text-3xl font-extrabold text-zinc-900 tracking-tight">Modus Penipuan Terbanyak</h2>
              </div>
            ...
            <div className="space-y-5 sm:space-y-6">
              {sortedCategories.map(([category, count], i) => {
                ...
                return (
                  <motion.div key={category} initial={{ opacity: 0, x: -10 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.06 }} className="group">
                    <div className="flex justify-between items-baseline mb-2">
                      <div className="flex items-center gap-2 sm:gap-3">
                        <span className="text-xs sm:text-sm font-mono text-zinc-300 w-5 sm:w-6">{String(i + 1).padStart(2, '0')}</span>
                        <span className="text-xs sm:text-sm font-bold text-zinc-900">{category}</span>
                      </div>
                      <span className="text-[11px] sm:text-xs font-bold text-zinc-400 tabular-nums">{count} laporan</span>
                    </div>
          ...
          <div className="max-w-6xl mx-auto px-4">
            <div className="flex flex-col sm:flex-row sm:items-end justify-between mb-8 sm:mb-10 gap-4">
              <div>
                ...
                <h2 className="text-2xl sm:text-3xl font-extrabold text-zinc-900 tracking-tight">Laporan Masuk</h2>
              </div>
              <Link href="/report" className="inline-flex items-center gap-2 text-sm font-bold text-zinc-900 hover:text-red-600 transition-colors self-start sm:self-auto">
                Buat Laporan <ArrowRight className="w-4 h-4" />
              </Link>
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4">
              {recentReports.map((report: any, i: number) => (
                <motion.div key={report.id} initial={{ opacity: 0, y: 10 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.05 }}>
                  <Link href={`/check/${report.target_number}`} className="block bg-white border border-zinc-200 rounded-xl p-4 sm:p-5 hover:shadow-md hover:border-zinc-300 transition-all group">
                    <div className="flex justify-between items-start mb-3">
                      <span className={`px-2 py-0.5 text-[10px] font-bold uppercase tracking-wider rounded ${
                        ...
                        report.status === 'rejected' ? 'bg-zinc-100 text-zinc-400' : 'bg-amber-50 text-amber-600'
                      }`}>
                    ...
                    <p className="text-base sm:text-lg font-extrabold text-zinc-900 mb-0.5 font-mono tracking-wide">{maskNumber(report.target_number)}</p>
