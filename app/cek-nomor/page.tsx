...
import {
  ArrowLeft, Phone, ShieldCheck, Clock, CheckCircle2, ArrowUpRight,
} from 'lucide-react';
...
export default async function CekNomorPage() {
  ...
  const [{ data: rawReports }, { count: totalPhone }, { count: verifiedPhone }] =
    await Promise.all([
      supabase.from('reports').select('id, target_number, target_name, category, created_at, status')
        .eq('target_type', 'phone').order('created_at', { ascending: false }).limit(6),
  ...
  return (
    <div className="min-h-screen bg-zinc-50">
      ...
      <div className="bg-white border-b border-zinc-100">
        <div className="max-w-5xl mx-auto px-4 py-10 sm:py-16 text-center">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, ease: [0.16, 1, 0.3, 1] }}>
            <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-blue-50 text-blue-600 text-[11px] font-bold uppercase tracking-widest mb-5 sm:mb-6">
              <Phone className="w-3.5 h-3.5" /> Cek Nomor HP / WhatsApp
            </div>
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.5, delay: 0.3 }}
            ...
            <div className="text-center">
              <p className="text-xl sm:text-2xl font-extrabold text-zinc-900">{totalPhone || 0}</p>
              <p className="text-[10px] sm:text-xs text-zinc-400 font-medium mt-1 uppercase tracking-wider">Nomor Dilaporkan</p>
            </div>
            <div className="w-px bg-zinc-200" />
            <div className="text-center">
              <p className="text-xl sm:text-2xl font-extrabold text-red-600">{verifiedPhone || 0}</p>
      ...
      {reports.length > 0 && (
        <div className="max-w-5xl mx-auto px-4 py-10 sm:py-14">
          <motion.div initial={{ opacity: 0, y: 10 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}
            ...
            <h2 className="text-xs sm:text-sm font-extrabold text-zinc-900 uppercase tracking-wider">Nomor Terbaru Dilaporkan</h2>
      ...
      <div className="max-w-5xl mx-auto px-4 pb-12 sm:pb-16">
        <motion.div initial={{ opacity: 0, y: 10 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}
          className="bg-blue-50 border border-blue-100 rounded-2xl p-6 sm:p-8">
          <div className="flex items-center gap-2 mb-4">
            <ShieldCheck className="w-5 h-5 text-blue-600 shrink-0" />
            <h3 className="text-xs sm:text-sm font-extrabold text-blue-900 uppercase tracking-wider">Tips Keamanan Nomor HP</h3>
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            {[
              ...
              'Jika menerima pesan mencurigakan, langsung blokir dan laporkan ke KawalTransaksi.',
            ].map((tip, i) => (
              <motion.div key={i} initial={{ opacity: 0, x: -10 }} whileInView={{ opacity: 1, x: 0 }}
                ...
                <p className="text-xs sm:text-sm text-blue-800 leading-relaxed">{tip}</p>
              </motion.div>
            ))}
          </div>
        </motion.div>
